...
def _is_budget_exceeded(exc):
    return exc.__class__.__name__ == 'QueryBudgetExceeded'
...
#         print 'lm', lm, 'qm', qm, kl_bernoulli(p, qm)
    ...
    def lucb(sample_fns, initial_stats, epsilon, delta, batch_size, top_n,
        ...
        t = 1
        # print(f"初始lucb化查询次数消耗:{query_init}")
        def update_bounds(t):
        ...
        while B > epsilon:  # 循环条件
            ...
            try:
                ...
                if _is_budget_exceeded(e):
                    # 预算耗尽：用目前的均值直接挑 top_n 作为结果
                    sorted_means = np.argsort(means)
                    return sorted_means[-top_n:]
                raise
            query_loop += 2 * batch_size
            t += 1
            ut, lt = update_bounds(t)
            B = ub[ut] - lb[lt]
            # print(f"query_loop::{query_loop}")
        sorted_means = np.argsort(means)
        return sorted_means[-top_n:]
    ...
    def anchor_beam(sample_fn, delta=0.05, epsilon=0.1, batch_size=10,
                    ...
                    verbose=False, epsilon_stop=0.1, min_samples_start=5,
                    max_anchor_size=3, verbose_every=1,
                    stop_on_first=False, coverage_samples=10000):
        anchor = {'feature': [], 'mean': [], 'precision': [],
                  'coverage': [], 'examples': [], 'all_precision': 0}
        # print(f"BEAMSEARCH IN SAMPLE_FN")
        ...
        def _find_min_flip_from_state(state):
            try:
                valid_n = state['current_idx']
                if valid_n <= 0:
                    return None
                labels_arr = state['labels'][:valid_n]
                if (labels_arr == 0).sum() == 0:
                    return None
                data_arr = state['data'][:valid_n]
                raw_arr = state['raw_data'][:valid_n].reshape(-1)
                changes = (data_arr.shape[1] - data_arr.sum(axis=1)).astype(int)
                flip_idx = np.where(labels_arr == 0)[0]
                i_min = flip_idx[np.argmin(changes[flip_idx])]
                return {'text': str(raw_arr[i_min]), 'changes': int(changes[i_min])}
            except Exception:
                return None
        ...
        while current_size <= max_anchor_size:
            ...
            # print(f"chosen_tuples{chosen_tuples}")
        ...
        anchor_out = AnchorBaseBeam.get_anchor_from_tuple(best_tuple, state)
        anchor_out['min_flip'] = _find_min_flip_from_state(state)

        # === NEW: aggregate per-feature importance from all sampled tuples ===
        feat_stats = collections.defaultdict(lambda: {'best_prec': 0.0, 'best_cov': 0.0, 'best_pc': 0.0})
        for t, ns in state['t_nsamples'].items():
            if ns <= 0:
                continue
            mean = state['t_positives'][t] / max(1.0, ns)    
            cov  = state['t_coverage'].get(t, 0.0)                 
            pc   = float(mean) * float(cov)                     
            for f in t:
                st = feat_stats[f]
                if mean > st['best_prec']:
                    st['best_prec'] = float(mean)
                if cov > st['best_cov']:
                    st['best_cov'] = float(cov)
                if pc > st['best_pc']:
                    st['best_pc'] = float(pc)
        anchor_out['feature_stats'] = {int(f): st for f, st in feat_stats.items()}
        return anchor_out